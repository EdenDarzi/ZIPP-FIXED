...
export default function HomePage() {
  ...
  useEffect(() => {
    async function fetchSuggestion() {
      ...
      try {
        const input: CulinaryAssistantInput = { userId: "mockUser123", currentDay: new Date().toLocaleString('he-IL', { weekday: 'long' }) }; // Use Hebrew locale
        ...
        setCulinarySuggestion("גלה את המסעדות המדהימות שלנו היום!"); 
      } finally {
    ...
    <div className="space-y-16">
      <section className="text-center py-16 bg-gradient-to-br from-primary/10 to-accent/10 rounded-xl shadow-xl overflow-hidden">
        <div className="animate-fadeInUp">
          ...
          <p className="text-xl text-foreground/80 max-w-2xl mx-auto mb-10">
            הפתרון האחד שלכם למשלוח מהיר ואמין מהמסעדות, החנויות ובתי הקפה המקומיים האהובים עליכם.
          </p>
        </div>
        <div className="max-w-xl mx-auto mb-10 animate-fadeInUp animation-delay-200">
          <div className="relative">
            <Search className="absolute right-4 top-1/2 -translate-y-1/2 h-5 w-5 text-muted-foreground" /> {/* Adjusted for RTL */}
            <Input
              type="search"
              placeholder="מה מתחשק לכם היום? (למשל, פיצה, סושי, 'משהו קליל')"
              className="w-full pr-12 pl-4 py-3 text-lg rounded-full shadow-lg focus:ring-2 focus:ring-primary focus:border-primary transition-all" // Adjusted padding for RTL
        ...
        <div className="space-x-4 animate-fadeInUp animation-delay-400">
          <Button size="lg" asChild className="bg-accent hover:bg-accent/90 text-accent-foreground shadow-lg transition-transform hover:scale-105 px-8 py-6 text-lg">
            <Link href="/restaurants">
              <span className="flex items-center justify-center w-full">
                <Utensils className="ml-2 h-5 w-5" /> {/* Icon on right for RTL */}
                חיפוש מסעדות 
              </span>
            </Link>
          </Button>
          <Button size="lg" variant="outline" asChild className="shadow-md transition-transform hover:scale-105 px-8 py-6 text-lg">
            <Link href="/auth/register">
               <span className="flex items-center justify-center w-full">
                 <ArrowLeft className="ml-2 h-5 w-5" /> {/* Icon on right for RTL */}
                 הרשמה 
               </span>
            </Link>
          </Button>
        ...
        <Card className="bg-primary/5 border-primary/20 shadow-lg hover:shadow-xl transition-shadow">
          <CardHeader>
            <CardTitle className="text-xl font-headline text-primary flex items-center">
              <Sparkles className="h-6 w-6 ml-2 text-yellow-500" /> {/* Icon on right for RTL */}
              העוזר הקולינרי שלך
            </CardTitle>
          </CardHeader>
          <CardContent>
            {isLoadingSuggestion ? (
              <p className="text-muted-foreground animate-pulse">חושב על משהו טעים בשבילך...</p>
            ) : (
      ...
      {recommendedForYou.length > 0 && (
        <section className="animate-fadeInUp animation-delay-800">
          <div className="flex items-center mb-6">
            <Heart className="h-7 w-7 ml-3 text-pink-500" /> {/* Icon on right for RTL */}
      ...
      {newInArea.length > 0 && (
        <section className="animate-fadeInUp animation-delay-1000">
          <div className="flex items-center mb-6">
            <MapPin className="h-7 w-7 ml-3 text-green-500" /> {/* Icon on right for RTL */}
      ...
      {recentlyViewedMock.length > 0 && (
         <section className="animate-fadeInUp animation-delay-1200">
          <div className="flex items-center mb-6">
            <History className="h-7 w-7 ml-3 text-blue-500" /> {/* Icon on right for RTL */}
      ...
      <section className="animate-fadeInUp animation-delay-1400">
        <div className="flex items-center mb-6">
          <Award className="h-7 w-7 ml-3 text-purple-500" /> {/* Icon on right for RTL */}
          <h2 className="text-3xl font-bold font-headline text-foreground">גלה את כל המסעדות</h2>
        </div>
        ...
        <div className="text-center mt-8">
            <Button variant="link" asChild className="text-lg text-primary hover:text-accent transition-colors">
                <Link href="/restaurants">
                  <span className="flex items-center justify-center w-full">
                    <ArrowLeft className="mr-1 h-4 w-4" /> הצג את כל המסעדות {/* Adjusted for RTL */}
                  </span>
                </Link>
            </Button>
        </div>
      ...
      <section className="grid md:grid-cols-3 gap-8 animate-fadeInUp animation-delay-1600">
        <Card className="hover:shadow-xl transition-all duration-300 hover:border-primary border-2 border-transparent">
          ...
          <CardContent className="text-center">
            <CardDescription>
              גלו מגוון רחב של מאכלים ומוצרים ממספר רב של עסקים מקומיים.
            </CardDescription>
          </CardContent>
        </Card>
        <Card className="hover:shadow-xl transition-all duration-300 hover:border-primary border-2 border-transparent">
        ...
        <Card className="hover:shadow-xl transition-all duration-300 hover:border-primary border-2 border-transparent">
          ...
          <CardContent className="text-center">
            <CardDescription>
              קבלו הצעות אישיות לפריטים ומסעדות המופעלות על ידי מנוע ה-AI שלנו.
            </CardDescription>
          </CardContent>
        </Card>
      </section>
      ...
      <section className="relative h-72 md:h-96 rounded-xl overflow-hidden shadow-2xl animate-fadeInUp animation-delay-1800">
        ...
        />
        <div className="absolute inset-0 bg-gradient-to-l from-black/60 via-black/30 to-transparent flex items-center"> {/* Adjusted gradient for RTL */}
