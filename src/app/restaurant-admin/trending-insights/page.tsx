...
import type { IdentifyDishOutputType } from '@/ai/flows/identify-dish-flow';

const mockTrendingInsights: IdentifyDishOutputType[] = [
	{
		identifiedDishName: 'Korean Corndogs with Cheese Pull',
		isTrend: true,
		trendSource: 'TikTok & Instagram',
		generalSuggestion:
			'משתמשים אוהבים את משיכת הגבינה הדרמטית! פריטים דומים ניתן למצוא בחיפוש \'נקניקיות גורמה\' או \'חטיפי גבינה מטוגנים\'.',
		businessOpportunity: {
			suggestedItemName: 'הקורנדוג הגבינתי האולטימטיבי של LivePick',
			suggestedPriceRange: '₪28-₪35',
			suggestedDescription:
				'קורנדוג פריך מצופה פנקו עם ליבת מוצרלה נמסה, מושלם למשיכת גבינה מספקת. מוגש עם רוטב לבחירה.',
			suggestedTags: ['חדש', 'להיט טיקטוק', 'גבינתי', 'אוכל רחוב'],
			rationale:
				'מעורבות גבוהה ברשתות חברתיות ומושך ויזואלית. יכול למשוך קהלים צעירים וקניות אימפולסיביות.',
		},
	},
	{
		...
		generalSuggestion:
			'טרנד \'האתגר החריף\' ממשיך. משתמשים המחפשים חריפות קיצונית יכולים לבדוק את אפשרויות הראמן החריפות ביותר שלנו.',
		businessOpportunity: {
			suggestedItemName: 'אתגר נודלס אינפרנו',
			suggestedPriceRange: '₪45-₪55 (כולל חלב קטן)',
			suggestedDescription:
				'העזו לנסות את הראמן הכי חריף שלנו אי פעם? עמוס בפלפלי רפאים ותערובת תבלינים סודית. סיימו אותו ותקבלו את תמונתכם על קיר התהילה שלנו!',
	...
	},
	{
		...
		generalSuggestion:
			'טרנד נוסטלגי שעדיין זוכה לחיפושים. הציעו גרסת גורמה או טוויסט עונתי.',
		businessOpportunity: {
			suggestedItemName: 'לאטה מוקצף ענן תשע',
			suggestedPriceRange: '₪18-₪24',
			suggestedDescription:
				'קפה מוקצף חלק וקטיפתי על חלב קר, עם נגיעת וניל וזילוף קרמל. קלאסיקה משודרגת.',
			suggestedTags: ['קפה', 'משקה טרנדי', 'אינסטגרמי'],
			rationale:
				'קל להכנה, רווחיות גבוהה ומושך ויזואלית לשיתוף חברתי. פונה לאוהבי קפה המחפשים משהו שונה.',
		},
	},
	{
		...
		isTrend: false,
		generalSuggestion:
			'מועדף תמידי. LivePick מציעה בתי קפה רבים עם אפשרויות טוסט אבוקדו מעולות. משתמשים יכולים לסנן לפי \'ארוחת בוקר\' או \'בית קפה\'.',
	},
];
...
export default function TrendingInsightsPage() {
	...
	const [insights, setInsights] = useState<IdentifyDishOutputType[]>(mockTrendingInsights);

	const handleQuickAddToMenu = (suggestion: IdentifyDishOutputType['businessOpportunity']) => {
			...
			<Card>
				...
				<CardContent>
					...
					<p className="text-sm text-muted-foreground p-2 bg-blue-50 border border-blue-200 rounded-md flex items-start">
						...
						<span>
							מוצגות הצעות טרנדים מבוססות על נתוני דמה. במערכת חיה, אלו יתעדכנו בזמן אמת ויכללו סטטיסטיקות כמו מספר חיפושים, שיתופים ובקשות דומות לכל טרנד.
						</span>
					</p>
				</CardContent>
			</Card>
				...
				{filteredInsights.map((insight, index) => (
					<Card key={index} className="shadow-lg hover:shadow-xl transition-shadow flex flex-col">
						<CardHeader className="pb-3">
							...
							{insight.trendSource && (
								<CardDescription className="text-xs">מקור טרנד (משוער): {insight.trendSource}</CardDescription>
							)}
						...
						{insight.businessOpportunity ? (
							<>
								<CardContent className="space-y-3 pt-0 pb-3 flex-grow">
									<div className="p-3 rounded-md bg-muted/40 border border-dashed">
										...
										<p className="text-sm">
											<strong>שם מוצע למנה:</strong> {insight.businessOpportunity.suggestedItemName}
										</p>
										<p className="text-sm">
											<strong>טווח מחיר מוצע:</strong> {insight.businessOpportunity.suggestedPriceRange}
										</p>
										<p className="text-sm">
											<strong>תיאור מוצע:</strong>{' '}
											<em>{insight.businessOpportunity.suggestedDescription}</em>
										</p>
										<div className="mt-2">
											<strong>תגיות מוצעות:</strong>
											<div className="flex flex-wrap gap-1 mt-1">
												{insight.businessOpportunity?.suggestedTags?.map((tag) => (
										...
										<div className="mt-3 relative aspect-video bg-gray-200 rounded overflow-hidden">
											<Image
												...
												objectFit="cover"
												data-ai-hint={
													insight.businessOpportunity.suggestedItemName
														?.toLowerCase()
														.split(' ')
														.slice(0, 2)
														.join(' ') || 'food concept'
											...
											/>
											<p className="absolute bottom-1 right-1 text-xs bg-black/50 text-white px-1 rounded">
												תצוגת AI (דמו)
											</p>
										</div>
										<p className="text-xs text-muted-foreground mt-2">
											<strong>נימוק:</strong> {insight.businessOpportunity.rationale}
										</p>
									</div>
								</CardContent>
								<CardFooter className="border-t pt-3 flex flex-col sm:flex-row gap-2">
									...
									<Button
										variant="outline"
										onClick={() => handleExplorePartnership(insight.identifiedDishName)}
							...
							<CardContent className="pt-0 pb-3 flex-grow">
								<p className="text-sm text-muted-foreground text-center py-4">
									אין הצעה עסקית ספציפית לטרנד זה כרגע.
								</p>
							</CardContent>
						)}
					</Card>
