...
import { DollarSign, ListOrdered, ShoppingCart, Users, AlertTriangle, Settings, LayoutList, Palette, BarChart3, Video, Info, CheckCircle, Loader2 } from "lucide-react";
    ...
    <div className="space-y-6">
      ...
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
        <Card>
          ...
          <CardContent>
            {stats.todayOrders === null ? <Loader2 className="h-6 w-6 animate-spin"/> : <div className="text-2xl font-bold">{stats.todayOrders}</div>}
        ...
        <Card>
          ...
          <CardContent>
            {stats.pendingOrders === null ? <Loader2 className="h-6 w-6 animate-spin text-orange-600"/> : <div className="text-2xl font-bold text-orange-600">{stats.pendingOrders}</div>}
            {stats.pendingOrders !== null && <p className="text-xs text-muted-foreground">נדרשת פעולה</p>}
          </CardContent>
        </Card>
        <Card>
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
          ...
          <CardContent>
             {stats.totalRevenueToday === null ? <Loader2 className="h-6 w-6 animate-spin text-green-600"/> : <div className="text-2xl font-bold text-green-600">₪{stats.totalRevenueToday.toFixed(2)}</div>}
            {stats.totalRevenueToday !== null && <p className="text-xs text-muted-foreground">+10% מאתמול (הדגמה)</p>}
          </CardContent>
        </Card>
        <Card>
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
            <CardTitle className="text-sm font-medium">פריט פופולרי</CardTitle>
            <ShoppingCart className="h-4 w-4 text-muted-foreground" />
          </CardHeader>
          <CardContent>
             {stats.popularItem === null ? <Loader2 className="h-6 w-6 animate-spin"/> : <div className="text-lg font-bold truncate">{stats.popularItem}</div>}
      ...
      <div className="grid md:grid-cols-2 gap-6">
          ...
          <CardContent className="space-y-3">
            <div className="p-3 border rounded-md bg-green-50 text-green-700">
             ...
             <div className="p-3 border rounded-md bg-yellow-50 text-yellow-700">
               {stats.newReviews === null ? <Loader2 className="h-4 w-4 animate-spin"/> : <p className="font-medium flex items-center"><Users className="mr-2 h-4 w-4"/> התקבלו {stats.newReviews} ביקורות חדשות לטיפולך</p>}
            </div>
        ...
        <Card>
          ...
          <CardContent className="text-center space-y-3">
            ...
            <Button variant="outline" onClick={() => toast({title: "הפעלת LiveKitchen", description: "השידור החי יתחיל. (הדגמה)"})}>הפעל שידור חי</Button>
