    ...
    <div className="flex justify-center items-center min-h-[calc(100vh-200px)] py-12" dir={currentLanguage === 'he' || currentLanguage === 'ar' ? 'rtl' : 'ltr'}>
      <Card className="w-full max-w-md shadow-xl">
        <CardHeader className="text-center">
          <CardTitle className="text-3xl font-headline text-primary">{t('welcomeBack', 'ברוכים השבים!')}</CardTitle>
          <CardDescription>{t('loginToContinue', 'התחבר כדי להמשיך ל-LivePick.')}</CardDescription>
        </CardHeader>
        <CardContent>
          <LoginForm />
          <div className="mt-6 text-center text-sm text-muted-foreground space-y-1">
            <p>
              ...
              <Button variant="link" asChild className="p-0 text-primary">
                <Link href="/auth/register"><span>{t('signUp', 'הירשם')}</span></Link>
              </Button>
            </p>
